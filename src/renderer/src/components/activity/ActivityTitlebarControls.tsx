...
export function ActivityTitlebarControls(): React.JSX.Element {
  // Why: the titlebar owns the persistent Orchestrator dialog subtree, so it must
  // rerender that open dialog when the application language changes.
  useTranslation()
    ...
    <div className="flex h-full min-w-0 flex-1 items-center gap-3 border-l border-border px-3">
      ...
      >
          ...
          <TooltipTrigger asChild>
            <Button
              ...
              aria-label={translate(
                'auto.components.activity.ActivityTitlebarControls.closeHarness',
                'Close Orchestrator'
              )}
            >
        ...
        <span className="truncate text-xs font-medium">
          {translate('auto.components.activity.ActivityTitlebarControls.harness', 'Orchestrator')}
        </span>
