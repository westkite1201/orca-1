...
import { SkillInstallOutcome, SkillInstallReview, SkillShareLinkInputForm } from './SkillInstallReviewContent'
...
export function SkillInstallDialog({
  ...
  const [executionTarget, setExecutionTarget] = useState<{ kind: 'wsl'; distro: string } | null>(null)
  ...
  const install = async (discardLocal = false): Promise<void> => {
    ...
    try {
      ...
      if (!discardLocal) {
        ...
        if (['modified', 'unowned', 'external-link', 'name-collision'].includes(checked.value.currentState)) {
          return
        }
      }
    ...
    <Dialog open={open} onOpenChange={(next) => !next && !busy && !bundleBusy && close()}>
      ...
      >
        <DialogHeader>
          <DialogTitle>
            {bundleVersion
              ? translate('auto.components.skills.SkillInstallDialog.01c5a14e01', 'Install shared skills')
              : translate('auto.components.skills.SkillInstallDialog.fcbec627cc', 'Install shared skill')}
          </DialogTitle>
        </DialogHeader>
        ...
        {resolvingInitialLink ? (
          ...
          <SkillShareLinkInputForm link={link} onLinkChange={setLink} onSubmit={() => void inspect()} />
        ) : bundleVersion ? (
        ...
        {!bundleVersion ? (
          <DialogFooter>
            ...
            {!preview && !resolvingInitialLink ? (
              <Button type="button" disabled={busy || !link.trim()} onClick={() => void inspect()}>
                ...
                {busy
                  ...
                  : translate('auto.components.skills.SkillInstallReviewContent.157de228b4', 'Inspect skill')}
              </Button>
            ) : null}
            ...
            {preview && (!result || ['conflict', 'partial', 'failed', 'cancelled'].includes(result.status)) ? (
              <Button
                type="button"
