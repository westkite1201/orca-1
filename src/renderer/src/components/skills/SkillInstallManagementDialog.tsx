...
import type { ManagedSkillInstall, SkillInstallResult } from '../../../../shared/skill-install-contract'
...
import { groupManagedSkillInstalls, type SkillManagedInstallGroup } from './skill-managed-install-groups'
...
export function SkillInstallManagementDialog({
  ...
  const selected = useMemo(() => groups.find((group) => group.key === selectedKey) ?? null, [groups, selectedKey])
  ...
  const installVersion = async (discardLocal = false): Promise<void> => {
    ...
    try {
      ...
      const bundleManifest = version?.manifest && 'skills' in version.manifest ? version.manifest : null
      if (bundleManifest) {
        const installedNames = new Set(selected.installs.map((install) => install.name))
        const selectedSkills = bundleManifest.skills.filter((skill) => installedNames.has(skill.name))
  ...
  const destructiveConflict =
    result?.status === 'conflict' || Boolean(selected?.installs.some((install) => install.state === 'modified'))
    ...
    <Dialog open={open} onOpenChange={(next) => !next && !busy && close()}>
      <DialogContent className="max-h-[calc(100vh-3rem)] overflow-x-hidden overflow-y-auto scrollbar-sleek sm:max-w-2xl [&>*]:min-w-0">
        ...
        {!busy && installs.length === 0 ? (
          <p className="rounded-md border border-border p-4 text-sm text-muted-foreground">{copy.noInstalls}</p>
        ) : null}
        {groups.length > 0 ? (
