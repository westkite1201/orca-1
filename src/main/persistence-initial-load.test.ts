...
describe('Store', () => {
  ...
  it('round-trips correlated Jaws and Harness runs', () => {
    ...
    const harness = store.createHarnessRun({
      mode: 'orchestrator',
      repoId: jaws.repoId,
      sourceWorktreeId: jaws.sourceWorktreeId,
      sourceWorktreePath: jaws.sourceWorktreePath,
      goal: plan.goal,
      verificationCommand: plan.verificationCommand,
      baseSha: jaws.baseSha,
      jawsRunId: jaws.id,
      approvedPlan: plan
    })
    store.attachJawsHarnessRun(jaws.id, harness.id)

    const reloaded = createStore()
    expect(reloaded.getJawsRun(jaws.id)?.harnessRunId).toBe(harness.id)
    expect(reloaded.getHarnessRun(harness.id)?.jawsRunId).toBe(jaws.id)
  })
