...
import {
  HarnessRunPersistence,
  installHarnessRunPersistenceContext
} from './harness-run-persistence'
import { JawsRunPersistence, installJawsRunPersistenceContext } from './jaws-run-persistence'
...
export type StoreDomains = {
  ...
  harnessRuns: HarnessRunPersistence
  jawsRuns: JawsRunPersistence
}
...
export function installStoreDomainContexts(target: object, domains: StoreDomains): void {
  ...
  installHarnessRunPersistenceContext(target, domains.harnessRuns)
  installJawsRunPersistenceContext(target, domains.jawsRuns)
...
export function createStoreDomains(runtime: StoreRuntimeState): StoreDomains {
  ...
  const harnessRuns = new HarnessRunPersistence(runtime, flushBarriers)
  const jawsRuns = new JawsRunPersistence(runtime)
  return {
    adaptation,
    backups,
    cohorts,
