...
const {
  ...
  registerJawsHandlersMock: vi.fn(),
...
vi.mock('../jaws', () => ({ registerJawsHandlers: registerJawsHandlersMock }))
...
describe('registerCoreHandlers', () => {
  beforeEach(() => {
    ...
    registerJawsHandlersMock.mockReset()
  ...
  it('passes the store through to handler registrars that need it', async () => {
    ...
    expect(registerJawsHandlersMock).toHaveBeenCalledWith(runtime)
