...
const {
  ...
  isRegisteredSshRuntimeReadyMock,
  ...
  isRegisteredSshRuntimeReadyMock: vi.fn(),
...
vi.mock('../../../ipc/ssh', () => ({
  ...
  isRegisteredSshRuntimeReady: isRegisteredSshRuntimeReadyMock,
...
describe('ssh RPC methods', () => {
  it('returns the registered SSH target state', async () => {
    ...
    isRegisteredSshRuntimeReadyMock.mockReturnValueOnce(true)
    ...
    expect(response).toMatchObject({ ok: true, result: { state, runtimeReady: true } })
  })
  ...
  it('returns null when the target has no registered state yet', async () => {
    ...
    isRegisteredSshRuntimeReadyMock.mockReturnValueOnce(false)
    const runtime = { getRuntimeId: () => 'test-runtime' } as unknown as OrcaRuntimeService
    const dispatcher = new RpcDispatcher({ runtime, methods: SSH_METHODS })
    ...
    expect(response).toMatchObject({
      ok: true,
      result: { state: null, runtimeReady: false }
    })
  })
