...
export function applySchemaMigrationsV13ToV30(this: OrchestrationDb, current: number): void {
  ...
  if (current < 31) {
    if (!this.hasColumn('tasks', 'verification_required')) {
      this.db.exec('ALTER TABLE tasks ADD COLUMN verification_required INTEGER NOT NULL DEFAULT 0')
    }
    if (!this.tasksStatusCheckAllowsReported()) {
      this.db.exec(`
        CREATE TABLE tasks_new (
          ...
          spec TEXT NOT NULL,
          status TEXT NOT NULL DEFAULT 'pending'
            CHECK(status IN ('pending', 'ready', 'dispatched', 'reported', 'completed', 'failed', 'blocked')),
        ...
        INSERT INTO tasks_new (
          ...
          created_by_process_incarnation, created_by_run_generation, task_title, display_name,
          verification_required, spec, status, deps, result, created_at, completed_at
        ) SELECT
          id, run_id, parent_id, created_by_terminal_handle, created_by_pane_key,
          created_by_process_incarnation, created_by_run_generation, task_title, display_name,
          verification_required, spec, status, deps, result, created_at, completed_at
