...
export function tasksStatusCheckAllowsReported(this: OrchestrationDb): boolean {
  const row = this.db
    .prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'tasks'")
    .get() as { sql: string } | undefined
  return !!row && row.sql.includes("'reported'")
}
...
export type SchemaColumnProbesMethods = {
  ...
  tasksStatusCheckAllowsReported: typeof tasksStatusCheckAllowsReported
}
...
export function attachSchemaColumnProbes(ctor: { prototype: object }): void {
  Object.assign(ctor.prototype, {
    ...
    tasksStatusCheckAllowsReported
  })
}
