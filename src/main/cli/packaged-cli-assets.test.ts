...
const productProfile = require('../../shared/product-profile.json') as {
  name: string
  userDataDirectoryName: string
}
const linuxLauncherAsset = new URL(
  `../../../resources/linux/bin/${productProfile.userDataDirectoryName}`,
  import.meta.url
)
...
const unixLauncherFixtures = [
  {
    ...
    appDir: [productProfile.name],
    launcher: ['resources', 'bin', productProfile.userDataDirectoryName],
    executable: [productProfile.userDataDirectoryName],
    cli: ['resources', 'app.asar.unpacked', 'out', 'cli', 'index.js']
  },
  {
    name: 'macOS',
    asset: darwinLauncherAsset,
    appDir: [`${productProfile.name}.app`],
    launcher: ['Contents', 'Resources', 'bin', 'orca'],
    executable: ['Contents', 'MacOS', productProfile.name],
...
`,
  ...
  itRunsUnixShell(
    ...
    async () => {
      ...
      try {
        const appDir = join(root, productProfile.name)
        const resourcesDir = join(appDir, 'resources')
        const launcherDir = join(resourcesDir, 'bin')
        const cliDir = join(resourcesDir, 'app.asar.unpacked', 'out', 'cli')
        const launcherPath = join(launcherDir, productProfile.userDataDirectoryName)
        const electronPath = join(appDir, productProfile.userDataDirectoryName)
...
`,
        ...
        const commandPath = join(commandDir, productProfile.userDataDirectoryName)
        await mkdir(commandDir, { recursive: true })
        await mkdir(join(homeDir, productProfile.userDataDirectoryName), { recursive: true })
